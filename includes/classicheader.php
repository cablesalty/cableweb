<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="/style/classic.css">
    <link rel="shortcut icon" href="/public/favicon.ico" type="image/x-icon">
    <script src="https://kit.fontawesome.com/2c53a1c7fb.js" crossorigin="anonymous"></script>
    <script src="/javascript/main.js" defer></script>
    <title>cablesalty</title>
</head>

<div class="header">
    <h1 class="title">cablesalty.com</h1>
    <h2 class="subtitle">under construction :: classic mode</h2>
    <div class="navbar">
        <a href="/">Homepage</a> | 
        <a href="/serverinfo">Server info</a> | 
        <a href="https://github.com/cablesalty">GitHub</a> | 
        <a href="">Discord</a> | 
        <a href="https://github.com/cablesalty/cableweb">View source code</a> | 
    </div>

    <h3 class="classicmodeicon" id="classicmodeicon">
        <it style="display: none" id="classicmodetext">Go back to the normal view</it>
        <i class="fa-solid fa-calendar-days"></i>
    </h3>
</div>
<hr>